...
class Document:
 ...
 def setPredicatesGivenSentence(self, sentence, position):
   predicates= []
   for p in srl.getSRL(srl, sentence.content):
     predicate= Predicate(p, position)
     predicates.append(predicate)
   sentence.setPredicates(predicates)
   sentence.setPosition(position)
   return sentence

 def setPredicatesFromSentences(self):
   predicates=[]
   sentencePosition= 1
   for s in self.getSentences():
     s= self.setPredicatesGivenSentence(s, sentencePosition)
     sentencePosition = sentencePosition + 1
     predicates.append(s.getPredicates())
   predicates=list(flatten(predicates))
